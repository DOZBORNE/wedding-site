...
export function drawVine(svg: SVGSVGElement, d: string, seed: number, spacing = 12, size = 1) {
...
function observed(el: HTMLElement, grow: () => void) {
 ...
 */
export function flankIvy(el: HTMLElement, opts: { seed?: number; crown?: boolean } = {}) {
...
export function dividerIvy(svg: SVGSVGElement, opts: { seed?: number } = {}) {
	try {
		...
		drawVine(svg, 'M 8 20 C 60 6 110 30 160 16 C 200 6 260 8 312 20', opts.seed ?? 10, 12, 0.8);
	} catch {
		/* decoration only */
	}
	return {};
